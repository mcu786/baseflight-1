...
// three receiver read functions
extern uint16_t pwmReadRawRC(uint8_t chan);
extern uint16_t spektrumReadRawRC(uint8_t chan);
extern uint16_t wifiReadRawRC(uint8_t chan); //And by DY										   
...
int main(void)
{
    ...
    pwm_params.enableInput = !feature(FEATURE_SPEKTRUM); // disable inputs if using spektrum
	 pwm_params.enableInput = !feature(FEATURE_WIFICONTROLLER); // Add by DY.disable inputs if using wifi controller
	...
	//And by DY
	if (feature(FEATURE_WIFICONTROLLER)){
		wifiInit();
	   	rcReadRawFunc = wifiReadRawRC;
	}else if (feature(FEATURE_SPEKTRUM)) {
        spektrumInit();
